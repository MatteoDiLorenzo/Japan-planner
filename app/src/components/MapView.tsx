...
import { X, Layers, Plus, Check, ExternalLink, Train, Bus, Eye, EyeOff, Navigation } from 'lucide-react';
...
const createBusIcon = (lineColor: string) => {
  return new DivIcon({
...
export function MapView({ 
  ...
  return (
    <div className="relative w-full h-full min-h-[600px] rounded-2xl overflow-hidden">
      ...
      >
        ...
        {showBus && cityBusStations.map((station) => (
          <Marker
            key={station.id}
            position={station.coordinates}
            icon={createBusIcon(station.lineColor)}
            eventHandlers={{
              click: () => setSelectedStation(station),
            }}
          >
      ...
      {showHotels && (
        <div className="absolute top-4 left-4 z-[400]">
          <Dialog open={showAddHotelDialog} onOpenChange={setShowAddHotelDialog}>
            ...
            <DialogContent className="bg-gray-900 border-white/10 max-w-md z-[10000]">
              <DialogHeader>
      ...
      {showMetro && (
        <div className="absolute top-4 right-4 z-[400]">
          ...
          >
            ...
            {showLineControls ? 'Nascondi Linee' : 'Mostra Linee'}
          </Button>
        </div>
      )}
      ...
      {/* Line Controls Panel */}
      {showLineControls && (
        <div className="absolute top-16 right-4 w-72 bg-black/95 backdrop-blur-md rounded-xl p-4 border border-white/20 z-[450] max-h-[70vh] shadow-2xl">
      ...
      {selectedStation && (
        <div className="absolute top-4 right-4 w-72 bg-black/90 backdrop-blur-md rounded-xl p-4 border border-white/10 z-[450] shadow-2xl">
      ...
      {selectedAttraction && (
        <div className="absolute top-4 right-4 w-80 bg-black/90 backdrop-blur-md rounded-xl p-4 border border-white/10 z-[450] shadow-2xl">
